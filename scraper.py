...
INPSECTION_PATH = '/health/ehs/foodsafety/inspections/Results.aspx?'
INSPECTION_PARAMS = {
    'Output': 'W',
    'Business_Name': '',
    'Business_Address': '',
...
def get_inspection_page(**kwargs):
    uri = INSPECTION_DOMAIN + INPSECTION_PATH
    ...
    # raises an error if there is an http error, otherwise no-op
    r.raise_for_status()

    # 'content' provides content in bytes
...
def parse_source(body, encoding='utf-8'):
    soup = BeautifulSoup(body, 'html5lib', from_encoding=encoding)
    return soup


def extract_data_listings(html):
    listings = html.find_all('div', id=re.compile(r'PR[\d]+~'))
    return listings
